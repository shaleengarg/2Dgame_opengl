...
void draw ()
{
    ...
    if(Jump == 1)
    {
        if(Human.z < MAX_JUMP && Human.direction == 1)
        {
            //cout << "if " << endl;
            Human.z += 0.3;
        }
        else if (Human.z >= MAX_JUMP || Human.direction == 0)
        {
            //cout << "elseif" << endl;
            Human.direction = 0;
            Human.z -= 0.3;
    ...
    for(i=0; i < No_cubes; i++)
    {
        ...
        if(glm::length(difference) < Human.radius)   //Resolution
        {
            ...
            if(Wx > 0) //collision form left Push towards +ve x
            {
                ...
                cout << abs(closest[0]-Human.radius) << endl;
            }
            else if (Wx < 0) //Push towards -ve x
            {
            ...
            else if (Wy < 0) //Push towards -ve y
            {
    ...
    if (Human.z < -6) // JUMP TO DEATH
    {
        cout << "Jump to death!!!" << endl;
        Human.z = MAP_CUBE_SIZE-2;
        Human.x = 0;
        Human.y = 0;
    }
